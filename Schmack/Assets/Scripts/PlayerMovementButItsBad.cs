...
public class PlayerMovementButitsbad : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        velocity.x = h * speed;
        WallStick();
        if((leftHit.collider != null && velocity.x < 0) || 
            (rightHit.collider != null && velocity.x > 0))
        {
            velocity.x = 0;
        }
    ...
    void Gravity()
    {
        ...
        if (downHit.collider != null && isFalling)
        {
            isJumping = false;
            isSticking = false;
            isFalling = false;
    ...
    {
        float distance = Mathf.Clamp(velocity.sqrMagnitude * 2f, 0.5f, float.MaxValue);
        Vector3 debug = new Vector3(direction.x, direction.y, 0);
        Debug.DrawLine(transform.position, transform.position + debug * distance, Color.magenta);
